...
import pytest
from flare_protobuf_plugin import additional_fields, python_protobuf_module_mapper
from flare_protobuf_plugin.python_protobuf_module_mapper import PythonProtobufMappingMarker
...
def test_map_first_party_modules_to_addresses_grpclib(rule_runner: RuleRunner) -> None:
    rule_runner.set_options(
        [
    ...
    rule_runner.write_files(
        {
            "protos/f.proto": "",
            "protos/BUILD": "protobuf_sources(grpc=True, python_protoc_plugin='grpclib')",
        }
    )
    result = rule_runner.request(FirstPartyPythonMappingImpl, [PythonProtobufMappingMarker()])
